 ...
 */
public class SixActivity extends Activity {  //FiveActivity类头部
    ...
    public void OnYS(View v) {    // 创建演示按钮，单击事件方法头部
        if (mdialog == null) {    // 如果自定义对话框为空
            ...
            mdialog = new mCustomProgressDialog(this, "演示中点击边缘取消", R.drawable.frame6);
        }
        mdialog.show();        // 显示对话框
    }   // 创建演示按钮，单击事件方法尾部
    ...
    private void initView() {  //initView()方法头部
        ...
        // 设置书写界面布局背景
        write_layout.setBackgroundResource(R.drawable.bg6);
        // 获取屏幕宽度
        ...
        try {
            // 通过输入流打开第一张图片
            InputStream is = getResources().getAssets().open("on6_1.png");
            // 使用Bitmap解析第一张图片
