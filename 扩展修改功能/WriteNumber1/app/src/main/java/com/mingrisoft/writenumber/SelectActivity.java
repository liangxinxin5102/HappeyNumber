...
public class SelectActivity extends Activity {   //SelectActivity类头部
    ...
    protected void onCreate(Bundle savedInstanceState) {   // onCreate()方法头部
        ...
        setContentView(R.layout.activity_selecty);
        if (MainActivity.isPlay==true){	//如果游戏主界面设置背景音乐为播放音乐状态
            PlayMusic();					//调用播放音乐的方法
        }

    }   //onCreate()方法尾部
    //    新添加一、二、三 点击事件进入相应书写界面
    public void OnYOne(View v){
        startActivity(new Intent(SelectActivity.this, OneYActivity.class));
    }
    public void OnYTwo(View v){
        startActivity(new Intent(SelectActivity.this, TwoYActivity.class));
    }
    public void OnYThree(View v){
        startActivity(new Intent(SelectActivity.this, ThreeYActivity.class));
    }
