 ...
 */
public class TwoYActivity extends Activity {    //OneActivity类头部
    ...
    public void OnYS(View v) {    // 创建演示按钮，单击事件方法头部
        if (mdialog == null) {    // 如果自定义对话框为空
            ...
            mdialog = new mCustomProgressDialog(this, "演示中点击边缘取消", R.drawable.framey2);
        }
        mdialog.show();        // 显示对话框
    }   // 创建演示按钮，单击事件方法尾部
    ...
    private void initView() {  //创建initView()方法头部
        ...
        try {
            // 通过输入流打开第一张图片
            InputStream is = getResources().getAssets().open("y2_1.png");
            // 使用Bitmap解析第一张图片
            ...
            public boolean onTouch(View v, MotionEvent event) {//手势按下判断的onTouch()方法
                switch (event.getAction()) {            // 获取行动方式头部
                    case MotionEvent.ACTION_DOWN:        // 手指按下事件
                        ...
                        if (x1 >= igvx + sw / 5 - 30 && x1 <= igvx + sw / 5 + 30
                                && y1 >= igvy + sh / 5 * 2 - sh / 10 / 2 & y1 <= igvy + sh / 5 * 2 + sh / 10 / 2
                                ) {
                            type = 1;                    // 开启书写
                        } else if (type == 2) {
                        } else {
                    ...
                    case MotionEvent.ACTION_MOVE:        // 手势移动中判断
                        ...
                        if (type == 1) {                    // 如果书写开启
                            if (y2 >= igvy + sh / 5 * 2 - sh / 10 / 2 & y2 <= igvy + sh / 5 * 2 + sh / 10 / 2) {
                                if (x2 > igvx + sw / 5 - 30 && x2 < igvx + sw / 5 * 3 / 7 * 1) {
                                    lodimagep(1);
                                } else if (x2 < igvx + sw / 5 * 3 / 7 * 2) {
                                    lodimagep(2);
                                } else if (x2 < igvx + sw / 5 * 3 / 7 * 3) {
                                    lodimagep(3);
                                } else if (x2 < igvx + sw / 5 * 3 / 7 * 4) {
                                    lodimagep(4);
                                } else if (x2 < igvx + sw / 5 * 3 / 7 * 5) {
                                    lodimagep(5);
                                } else if (x2 < igvx + sw / 5 * 3 / 7 * 6) {
                                    lodimagep(6);
                                } else if (x2 < igvx + sw / 5 * 3 / 7 * 7) {
                                    lodimagep(7);
                                }
                            } else {
                        ...
                        if (type == 2) {
                            if (y2 >= igvy + sh / 5 * 3 -30 & y2 <= igvy + sh / 5 * 3+30) {
                                if (x2 > igvx - 30 && x2 < igvx + sw / 18 * 1) {
                                    lodimagep(8);
                                } else if (x2 < igvx + sw / 18 * 2) {
                                    lodimagep(9);
                                } else if (x2 < igvx + sw / 18 * 3) {
                                    lodimagep(10);
                                } else if (x2 < igvx + sw / 18 * 4) {
                                    lodimagep(11);
                                } else if (x2 < igvx + sw / 18 * 5) {
                                    lodimagep(12);
                                } else if (x2 < igvx + sw / 18 * 6) {
                                    lodimagep(13);
                                } else if (x2 < igvx + sw / 18 * 7) {
                                    lodimagep(14);
                                } else if (x2 < igvx + sw / 18 * 8) {
                                    lodimagep(15);
                                } else if (x2 < igvx + sw / 18 * 9) {
                                    lodimagep(16);
                                } else if (x2 < igvx + sw / 18 * 10) {
                                    lodimagep(17);
                                } else if (x2 < igvx + sw / 18 * 11) {
                                    lodimagep(18);
                                } else if (x2 < igvx + sw / 18 * 12) {
                                    lodimagep(19);
                                } else if (x2 < igvx + sw / 18 * 13) {
                                    lodimagep(20);
                                } else if (x2 < igvx + sw / 18 * 14) {
                                    lodimagep(21);
                                } else if (x2 < igvx + sw / 18 * 15) {
                                    lodimagep(22);
                                } else if (x2 < igvx + sw / 18 * 16) {
                                    lodimagep(23);
                                } else if (x2 < igvx + sw / 18 * 17) {
                                    lodimagep(24);
                                } else if (x2 < igvx + sw / 18 * 18) {
                                    lodimagep(25);
                                }
                            } else {
    ...
    protected void dialog() {        // 完成后提示对话框头部
        ...
        // 实例化对话框
        AlertDialog.Builder builder = new AlertDialog.Builder(TwoYActivity.this);
