 ...
 */
public class OneYActivity extends Activity {    //OneActivity类头部
    ...
    public void OnYS(View v) {    // 创建演示按钮，单击事件方法头部
        if (mdialog == null) {    // 如果自定义对话框为空
            ...
            mdialog = new mCustomProgressDialog(this, "演示中点击边缘取消", R.drawable.framey1);
        }
        mdialog.show();        // 显示对话框
    }   // 创建演示按钮，单击事件方法尾部
    ...
    private void initView() {  //创建initView()方法头部
        ...
        try {
            // 通过输入流打开第一张图片
            InputStream is = getResources().getAssets().open("y1_1.png");
            // 使用Bitmap解析第一张图片
            ...
            public boolean onTouch(View v, MotionEvent event) {//手势按下判断的onTouch()方法
                switch (event.getAction()) {            // 获取行动方式头部
                    case MotionEvent.ACTION_DOWN:        // 手指按下事件
                        ...
                        if (x1 >= igvx - 30 && x1 <= igvx + 30
                                && y1 >= igvy + sh / 2 - sh / 8 / 2 - 50 & y1 <= igvy + sh / 2 + sh / 8 / 2 + 50
                                ) {
                            type = 1;                    // 开启书写
                        } else {
                            type = 0;                    // 否则关闭书写
                    ...
                    case MotionEvent.ACTION_MOVE:        // 手势移动中判断
                        ...
                        if (type == 1) {                    // 如果书写开启
                            if (y2 >= igvy + sh / 2 - sh / 8 / 2 - 50 & y2 <= igvy + sh / 2 + sh / 8 / 2 + 50) {
                                if (x2 >= igvx && x2 <= igvx + sw / 25 * 1) {
                                    lodimagep(1);
                                } else if (x2 <= igvx + sw / 25 * 2) {
                                    lodimagep(2);
                                } else if (x2 <= igvx + sw / 25 * 3) {
                                    lodimagep(3);
                                } else if (x2 <= igvx + sw / 25 * 4) {
                                    lodimagep(4);
                                } else if (x2 <= igvx + sw / 25 * 5) {
                                    lodimagep(5);
                                } else if (x2 <= igvx + sw / 25 * 6) {
                                    lodimagep(6);
                                } else if (x2 <= igvx + sw / 25 * 7) {
                                    lodimagep(7);
                                } else if (x2 <= igvx + sw / 25 * 8) {
                                    lodimagep(8);
                                } else if (x2 <= igvx + sw / 25 * 9) {
                                    lodimagep(9);
                                } else if (x2 <= igvx + sw / 25 * 10) {
                                    lodimagep(10);
                                } else if (x2 <= igvx + sw / 25 * 11) {
                                    lodimagep(11);
                                } else if (x2 <= igvx + sw / 25 * 12) {
                                    lodimagep(12);
                                } else if (x2 <= igvx + sw / 25 * 13) {
                                    lodimagep(13);
                                } else if (x2 <= igvx + sw / 25 * 14) {
                                    lodimagep(14);
                                } else if (x2 <= igvx + sw / 25 * 15) {
                                    lodimagep(15);
                                } else if (x2 <= igvx + sw / 25 * 16) {
                                    lodimagep(16);
                                } else if (x2 <= igvx + sw / 25 * 17) {
                                    lodimagep(17);
                                } else if (x2 <= igvx + sw / 25 * 18) {
                                    lodimagep(18);
                                } else if (x2 <= igvx + sw / 25 * 19) {
                                    lodimagep(19);
                                } else if (x2 <= igvx + sw / 25 * 19) {
                                    lodimagep(19);
                                } else if (x2 <= igvx + sw / 25 * 21) {
                                    lodimagep(21);
                                } else if (x2 <= igvx + sw / 25 * 22) {
                                    lodimagep(22);
                                } else if (x2 <= igvx + sw / 25 * 23) {
                                    lodimagep(23);
                                } else if (x2 <= igvx + sw / 25 * 24) {
                                    lodimagep(24);
                                } else if (x2 <= igvx + sw / 25 * 25) {
                                    lodimagep(25);
                                } else {
                                    type = 0;         // 手指离开 设置书写关闭
                                }
    ...
    protected void dialog() {        // 完成后提示对话框头部
        ...
        // 实例化对话框
        AlertDialog.Builder builder = new AlertDialog.Builder(OneYActivity.this);
